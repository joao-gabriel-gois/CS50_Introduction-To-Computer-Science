...
void blur(int height, int width, RGBTRIPLE image[height][width]) {
  ...
  for (int c = 0; c < width; c++) {
    for (int r = 0; r < height; r++) image_copy[r][c] = image[r][c];
  }
  ...
  for (int c = 0; c < width; c++) {
    for (int r = 0; r < height; r++) {
      ...
      // looping through every round pixel (3x3 box with current pixel in center)
      for (int i = 0; i < PIXEL_BLUR_RANGE; i++) {
        ...
        // if we are in a valid pixel (inside image area, sum it and count the valid pixel)
        if (curr_column_around != -1 && curr_row_around != -1) {
          // getting value from copy to get always non-modified values
