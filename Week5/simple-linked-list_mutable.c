...
int main(void) {
  ...
  // Filling
  for (int i = 0; i < 9; i++) insert(&list1, i);
  for (int i = 18; i > 0; i -= 2) insert(&list2, i);
...
bool remove_first(node** list, int number) {
  if (*list == NULL) return false; // with semicolon it breaks with " ‘else’ without a previous ‘if’"
...
bool remove_all(node** list, int number) {
  ...
  if (!removed && !has_remaining_elements) return false;
  else if (!has_remaining_elements) return true;
