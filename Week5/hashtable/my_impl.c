...
unsigned int hash(char *str) {
  ...
  return (char_code_sum + counter + 1) % HASH_MAX; // making string length relevant for diff hash
}
...
// If addition order doesn't matters, we can use only head with singly linked lists
