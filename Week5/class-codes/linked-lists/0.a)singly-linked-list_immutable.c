// As per CS50 Class, singly linked lists are not recommended
//   for removing a single element for the list. I could handle
//   it by using a pointer to a pointer, so even deleting, the removed
//   node keeps its address to a pointer that now will point to next element
//   so nodes before it will not lose reference to it
