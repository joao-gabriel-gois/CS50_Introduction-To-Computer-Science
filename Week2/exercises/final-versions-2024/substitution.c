...
char get_char_cipher(char c, string key)
{
    ...
    // With char AASCI number - a (or A if it is an uppercase char)
    // we can get 0 if char is a, and increment in one to reach next
    // letter. This way we can find index in key string for that letter
    int delta_to_index = is_char_lower ? 'a' : 'A';
    int char_index_in_key_string = is_letter(c) ? c - delta_to_index : -1;

    if (char_index_in_key_string == -1)
    {
        return c;
    }
    ...
    return is_char_lower ? tolower(returned_char) : toupper(returned_char);
}
