...
void lock_pairs(void) {
  for (int i = 0; i < pair_count; i++) {
    ...
    if (current_winner == current_loser) continue;
    ...
    for (int j = 0; j < i; j++) {
      ...
      if (current_loser != previous_winner) {
        is_lockable = (
          is_lockable && !locked[current_loser][previous_winner]
        );
      }
    }

    if (is_lockable) locked[current_winner][current_loser] = true;
  }
  return;
}
...
void print_winner(void) {
  ...
  for (int i = 0; i < candidate_count; i++) {   
    ...
    for (int j = 0; j < candidate_count; j++) {
      if (i != j) none_locked = none_locked && !locked[j][i];
    }
  ...
  if (winner_index >= 0) printf("%s\n", candidates[winner_index]);

  return;
}

// Utils
int get_pair_strenth(pair p) { return preferences[p.winner][p.loser]; }
