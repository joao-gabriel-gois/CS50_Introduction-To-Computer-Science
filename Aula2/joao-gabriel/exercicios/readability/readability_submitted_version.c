...
/**
  It will respond the inpuuted text grade level based on
    Coleman-Liau index.
*/
...
int count_words(string text)
{
    ...
    for (int i = 0; i < text_length; i++)
    {
        ...
        if (is_letter(text[i]) && is_previous_a_space &&)
        {
            counter++;
        }
        else if (is_previous_a_space && is_current_not_a_space && !is_letter(text[i]) && is_next_a_letter)
        {
            counter++;
        }
    }
...
int count_sentences(string text)
{
    ...
    for (int i = 0; i < text_length; i++)
    {
        bool is_sentence_end_marker = (text[i] == '!' || text[i] == '?' || text[i] == '.') ? 1 : 0;

        bool is_previous_char_letter = i > 0 && is_letter(text[i - 1]);

        if (is_sentence_end_marker && is_previous_char_letter)
        {
            counter++;
        }
    }
