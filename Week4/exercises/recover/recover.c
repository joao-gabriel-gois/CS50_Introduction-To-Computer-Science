...
void get_images_from_input(FILE* input) {
  ...
  while (fread(&chunk, CHUNK_SIZE, 1, input)) {
    bool is_chunk_containing_header = (
      chunk[0] == 0xFF && chunk[1] == 0xD8
        && chunk[2] == 0xFF && (chunk[3] & 0xF0) == 0xE0
    );
    // CHECK IF IT IS A JPG HEADER
    if (is_chunk_containing_header) {
      if (is_writting) {
        fclose(output);
        free(filename);
        is_writting = false;
      ...
      // set incremental (n) filename (for diff. imgs)
      filename = malloc(sizeof("outputs/000.jpg"));
      sprintf(filename, "outputs/%03i.jpg", n);
